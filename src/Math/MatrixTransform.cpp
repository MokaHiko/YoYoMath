...
namespace yoyo
{
    Mat4x4 OrthographicProjectionMat4x4(real left, real right, real bottom, real top, real near, real far)
    {
        Mat4x4 out_matrix = {};

        real rl = 1.0f / (right - left);
        real tb = 1.0f / (top - bottom);
        real fn = 1.0f / (far - near);
    ...
    Mat4x4 PerspectiveProjectionMat4x4(real fov_radians, real aspect_ratio, real near, real far)
    {
        Mat4x4 out_matrix = {};
        real half_tan_fov = Tan(fov_radians * 0.5f);
    ...
    Mat4x4 RotateEulerX(real angle_radians) {
        Mat4x4 out_matrix = {};
    ...
    Mat4x4 RotateEulerY(real angle_radians)
    {
        Mat4x4 out_matrix = {};
        real c = Cos(angle_radians);
        real s = Sin(angle_radians);
    ...
    Mat4x4 RotateEulerZ(real angle_radians) {
        Mat4x4 out_matrix = {};
    ...
    YAPI Mat4x4 InverseMat4x4(const Mat4x4& matrix)
    {
        ...
        real d = 1.0f / (m[0] * o[0] + m[4] * o[1] + m[8] * o[2] + m[12] * o[3]);
	...
	YAPI Quat RotationFromMat4x4(const Mat4x4& matrix)
	{
        ...
        for(int i = 0; i < 3; i++)
        {
            real scale_factor = rot_mat[0 + i ] * rot_mat[0 + i] + rot_mat[4 + i] * rot_mat[4 + i] + rot_mat[8 + i] * rot_mat[8 + i];
            real inverse_scale_factor = 1.0f / scale_factor;
