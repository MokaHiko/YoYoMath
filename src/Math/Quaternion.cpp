...
namespace yoyo
{
	...
	{
		real normal = NormalQuat(q);
		return { q.x / normal, q.y / normal, q.z / normal, q.w / normal };
	}

	YAPI Quat QuatFromAxisAngle(const Vec3& axis, real angle, bool normalize)
	{
		const real half_angle = 0.5f * angle;
	...
	YAPI Vec3 EulerAnglesFromQuat(const Quat& q)
	{
		...
		*/
		real t0 = 2.0 * (q.w * q.x + q.y * q.z);
		real t1 = 1.0 - 2.0 * (q.x * q.x + q.y * q.y);
		real roll_x = atan2(t0, t1);

		real t2 = +2.0 * (q.w * q.y - q.z * q.x);
		t2 = t2 > 1.0 ? 1.0 : t2;
		t2 = t2 < -1.0 ? -1.0 : t2;
		real pitch_y = asin(t2);

		real t3 = 2.0 * (q.w * q.z + q.x * q.y);
		real t4 = 1.0 - 2.0 * (q.y * q.y + q.z * q.z);
		real yaw_z = atan2(t3, t4);
	...
	YAPI const Quat Slerp(const Quat& q1, const Quat& q2, real t)
	{
		...
		real dot = q1_normalized.x* q2_normalized.x + q1_normalized.y * q2_normalized.y +
		...
		// Interpolate
		real s1 = std::cos(theta) - dot * std::sin(theta) / std::sin(theta_0);
		real s2 = std::sin(theta) / std::sin(theta_0);
